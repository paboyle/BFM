/* $Id: arralloc.C,v 1.1.1.1 2009/04/07 10:40:11 peterboyle Exp $ */
...
#define MIN_ALIGN 1
#endif
char RCSid[]="$Id: arralloc.C,v 1.1.1.1 2009/04/07 10:40:11 peterboyle Exp $";


/*----------------------------------------------------------------------*/


void 	subarray(size_t align_size, size_t size, int ndim, 
                 int prdim, void ***pp,void **qq, int *dimp, int index)
...
void *arralloc(size_t size, int ndim, ...)
{
    ...
    */
   if ( (dimp=(int *)malloc( ndim * sizeof(int) )) == NULL )
	return 0;
   bzero((char *)dimp,ndim * sizeof(int));
   ...
   if( (start = (void**)malloc(
      ...
      return 0;
   bzero((char *)start,(size_t)((n_data*size)+align_size+(n_ptr*sizeof(void**)))); 
