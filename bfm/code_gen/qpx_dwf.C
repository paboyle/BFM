...
void dwf_dslash( char *name)
{
  ...
  if ( 1)
  {
    ...
    for(int dir=0;dir<8;dir++){
      fill_l2t(U_p_s,0,9,CACHE_MANAGEMENT_PSI); 
  ...
  { 
	...
	if(compute_pointers){
	  ...
	  if ( dir ) {
	    fill_l1t_l2t(Chi_mu,12,CACHE_MANAGEMENT_PSI); 
	  }
	} else {
	...
	if( MIC_CACHE_MANAGEMENT ) {
	  /*
	  if (mu==0 && (pmdir==1) ) {						 
	    fill_l1t(Chi_mu,24,CACHE_MANAGEMENT_PSI);  // mostly already in l2
	  } else if (mu==1) {					 
	    if (pmdir) fill_l1t_l2t(Chi_mu,24,CACHE_MANAGEMENT_PSI);  // pull in + direction
	    else       fill_l1t_l2t(Chi_mu,24,CACHE_MANAGEMENT_PSI);                  // already in when - dir
	  } else if (mu==2) {					 
	...
	//	fill_l1t(U_p_s,9,CACHE_MANAGEMENT_PSI);
	//	make_inst(IALUPIPE,IOR,U_p_s,U_p_s,U_p_s);
	...
	if ( !compute_pointers ){
	  queue_iload_short(temp4,TAB_IMM[(dir+1)%8],tab);  
	  queue_iadd(temp4,temp4,temp5);
	  if(dir<7){
	    fill_l1t(temp4,0,24,CACHE_MANAGEMENT_PSI); 
	  } else {
	    fill_l1t(temp4,24,48,CACHE_MANAGEMENT_PSI); 
	  }

	  queue_iload_short(temp4,TAB_IMM[(dir+4)%8],tab);  
	  queue_iadd(temp4,temp4,temp5);
	  if(dir<4){
	    fill_l2t(temp4,0,12,CACHE_MANAGEMENT_PSI); 
	  } else {
	    fill_l1t(temp4,12,24,CACHE_MANAGEMENT_PSI); 
	  }
	  exit(0);
	}
	/*****************************************************************
	...
	queue_imul(temp5,temp4,temp5);
	queue_iadd(Chi_mu,temp5,Chi_mu);                                         // Base+s*Mult
	fill_l1t_l2t(Chi_mu,12,CACHE_MANAGEMENT_PSI);  // mostly already in l2
      }
    }
