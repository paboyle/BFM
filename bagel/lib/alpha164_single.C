...
struct processor *create_alpha164s (void)
{
  ...
  proc->CacheLine = 32; /*32  cache line*/
  proc->npipes=4;
  ...
  pipes[3].issuegroups = IALUMASK | BRCHMASK | LOADMASK
                       | FLODMASK | CACHMASK;
  ...
  create_alpha164s_instructions(proc);
  return(proc);
}
...
void check_iterations_alpha164s(int cntreg,int retno)
{
