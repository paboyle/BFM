 ...
 *  and may not be edistributed without permission.
...
int main ( int argc, char *argv[])
{
  ...
  while ( ( arg = getopt(argc,argv,"aRln:P:ds")) != EOF){
    switch(arg){
...
void dwf_dslash( char *name)
{
  ...
  queue_iload(twospinor_base,ZERO_IMM,args);
   ...
   * and su3 multiply it                                          *
   ****************************************************************/
  queue_iadd_imm(chidrain,twospinor_base,ZERO_IMM);
  ...
  for ( int mu=0;mu<4;mu++ ) {
   for ( int pmdir=0;pmdir<2;pmdir++ ) {
    ...
    queue_load_addr(tab,Isize,tab); // Always increment table inside the mu/pm loop
                                    // Chinxt may not be used if we break to the 
                                    // reconstruct, but tab will be reused to load Chi_mu above
    queue_iload(chinxt,ZERO_IMM,tab);  
   ...
   */
    ...
    queue_iadd_imm(pref,s,m1);
    queue_iadd_imm(Chi_mu,Chi_mu,PSI_IMM);
    int skipit = get_target_label();
    conditional_branch_cmpzero ( BRANCH_GT, pref, skipit  ) ;
    if ( (mu == 3) && (pmdir == 1) ) { 
      queue_iadd_imm(Chi_mu,Psi_p,ZERO_IMM);
    } else { 
      queue_iadd_imm(Chi_mu,chinxt,ZERO_IMM);
    }
    make_inst(DIRECTIVE,CmovTarget,skipit); // replace with cmov
    pragma(DCBT_SPACE,0);
    ...
    // Could immediately drop some DCBT's into the LFB's ???

    for(co=0;co<3;co++) complex_store(UChi[1][co],CHIIMM[1][co][0],chidrain,FourSpinType);
    ...
    queue_iadd_imm(chidrain,twospinor,ZERO_IMM);

    for(co=0;co<3;co++) complex_store(UChi[0][co],CHIIMM[0][co][0],chidrain,FourSpinType);
