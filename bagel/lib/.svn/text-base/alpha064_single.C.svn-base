...
struct processor *create_alpha064s (void)
{
  ...
  pipes[0].issuegroups = FADDMASK|FMOVMASK | FMULMASK ;
  pipes[1].issuegroups = IALUMASK | BRCHMASK | LOADMASK  | FSTRMASK
  ...
  create_alpha064s_instructions(proc);
  return(proc);
}

void create_alpha064s_instructions( struct processor *proc)
{
  int i,j;
  ...
  /*Define the FADDS*/
  fadds[FADD].mnemonic = "adds  ";
  fadds[FSUB].mnemonic = "subs  ";
  for (i=0;i<nfadds;i++){
    fadds[i].narg = 3;
...
int start_loop_alpha064s(int counter)
{
...
void stop_loop_alpha064s(int branchno,int counter)
{
  queue_load_addr(counter,Hdecrement,counter);
  make_inst(BRCHPIPE,BRANCH_GT,counter,branchno);
}
void check_iterations_alpha064s(int cntreg,int retno)
{
  make_inst(BRCHPIPE,BRANCH_LE,cntreg,retno);      /*Branch if cntreg <= 0*/
}
/* ...} */

void directive_print_alpha064s(struct instruction *i)
{
    switch ( i->code ) {
    case Target:
      printf("%s:\n",make_label(i->args[0]));
...
void queue_cmovge_alpha064s(int cond,int src,int dst)
{
  make_inst(IALUPIPE,CMOVGE,dst,cond,src);
}

void cache_print_alpha064s(struct instruction *i)
{
