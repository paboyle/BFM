/*
 *
 *  Copyright Peter Boyle and the University of Glasgow 2000.
 *
...
struct processor *create_powerIIIs ()
{
  struct regs *iregs=new regs;
  struct regs *fregs=new regs;
  struct processor *proc = new processor;
  struct pipeline *pipes = new pipeline[4]; /*Hardwire 3 pipes for now!!*/
  ...
  pipes[1].issuegroups = FADDMASK | FMULMASK | FMACMASK |FMOVMASK ;
  ...
  create_powerIIIs_instructions(proc);
  return(proc);
}
