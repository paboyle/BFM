						...
						int single)
{
  ...
  for(int s=0;s<nshift;s++){
    ...
    this->check_is_same(mresidual[s],(char *)"mresid");

    rsq[s] = cp * mresidual[s] * mresidual[s];
    if ( isBoss() && (!me) && verbose ) 
      printf("bfmbase::CGNE_prec_multi: shift %d target resid %le %le %le\n",s,rsq[s],cp,mresidual[s]);
    copy(ps[s],src);
  }
  // r and p for primary
  copy(r,src);
...
//not correct yet
  ...
  z[0][iz]   = 1.0;
  bs[0]      = b;
    this->check_is_same(mass[0],"mass");
  for(int s=1;s<nshift;s++){
    this->check_is_same(mass[s],"mass_s");
    z[s][1-iz] = 1.0;
    z[s][iz]   = 1.0/( 1.0 - b*(mass[s]-mass[0]));
    bs[s]      = b*z[s][iz]; // Sign relative to Mike - FIXME
    this->check_is_same(bs[s],(char *)"bs before entry");
  }
  ...
  for (k=1;k<=max_iter;k++){
   ...
   d += rn*mass[0];
	...
	double z0 = z[s][1-iz];
	double z1 = z[s][iz];
	z[s][iz] = z0*z1*bp  / (b*a*(z1-z0) + z1*bp*(1- (mass[s]-mass[0])*b)); 
	bs[s] = b*z[s][iz]/z0; // NB sign  rel to Mike
	this->check_is_same(z[s][iz],(char *)"zs");
	this->check_is_same(bs[s],(char *)"bs");
      }
    }
    ...
    // Convergence checks
    int all_converged = 1;
    if ( ((k%1)==0) && isBoss() && (!me) ) printf("bfmbase::CGNE_prec_multi: k=%d c=%g\n",k,c);
    for(int s=0;s<nshift;s++){
	...
	this->check_is_same(c,(char *)"c");
	this->check_is_same(css,(char *)"css");

	if(css<rsq[s]){
	  converged[s]=1;
	  //if ( isBoss() && (!me) ) printf("bfmbase::CGNE_prec_multi: k=%d Shift %d has converged\n",k,s);
	  if ( (!me) ) printf("bfmbase::CGNE_prec_multi: k=%d Shift %d has converged css %le rsq %le\n",k,s);
	} else {
	  all_converged=0;
	}
    ...
    if ( all_converged ){
      if ( (!me) )printf("bfmbase::CGNE_prec_multi: k=%d All shifts have converged\n",k);
      if ( (!me) )printf("bfmbase::CGNE_prec_multi: k=%d Checking solutions\n",k);
      // Check answers 
      for(int s=0; s < nshift; s++) { 
  ...
  if ( isBoss() && !me ) printf("bfmbase::CGNE_prec_multi: CG not converged after %d iterations\n",k);
  for(int s=0; s < nshift; s++) { 
    Mprec(psi[s],mp,tmp,DaggerNo);
    Mprec(mp,mmp,tmp,DaggerYes);
    axpy(tmp,psi[s],mmp,mass[s]);
