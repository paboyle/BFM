...
void create_bgq_instructions( struct processor *proc)
{
  ...
  fmovs[FSPLAT].mnemonic = "qvesplati  ";
  for (i=0;i<nfmovs;i++){
    fmovs[i].narg = 2;
  ...
  fmovs[FSPLAT].narg = 3;
  fmovs[FSPLAT].inout[2] = outarg;
  fmovs[FSPLAT].argtypes[2] = SIMM;
  ...
  floads[CLOAD_INDEXED].mnemonic = "qvlfdxa  " ;
  floads[CSLOAD_INDEXED].mnemonic = "qvlfsxa  " ;
  floads[CLOAD_SPLAT_INDEXED].mnemonic = "qvlfcdxa  " ;
  for (i=0;i<nfloads;i++){
    floads[i].narg = 3;
  ...
  floads[CLOAD_SPLAT_INDEXED].argtypes[1]=Iregs;
  floads[CSLOAD_SPLAT_INDEXED].argtypes[1]=Iregs;
  ...
  fsaves[CSSAVE_INDEXED].mnemonic = "qvstfsx " ;
  for (i=0;i<nfsaves;i++){
    fsaves[i].narg = 3;
