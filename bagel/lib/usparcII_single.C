...
struct processor *create_usparcIIs (void)
{
  ...
  struct pipeline *pipes = new pipeline[6]; /*Hardwire 4 pipes for now!!*/
  ...
  proc->delayslot = 1;
  proc->RegWindows = 1;
  proc->Bias = 0; /*v8plus mode has no implicit Bias*/
  ...
  proc->ReturnPointer = 31;
    ...
    /*
     * If I choosing to not use the register windows and
  ...
  create_usparcIIs_instructions(proc);
  return(proc);
}
...
void create_usparcIIs_instructions( struct processor *proc)
{
  ...
  /*Define the FMOVS*/
  fmovs[FMOV].mnemonic = "fmovs  ";
  fmovs[FNEG].mnemonic = "fnegs  ";
  for (i=0;i<nfmovs;i++){
    fmovs[i].narg = 2;
  ...
  floads[FLOAD_EA].mnemonic = "ld "  ;
  floads[FLOAD_REG_EA].mnemonic = "ld "  ;
  for (i=0;i<nfloads;i++){
    floads[i].narg = 3;
  ...
  fsaves[FSAVE_EA].mnemonic = "st " ;
  fsaves[FSAVE_REG_EA].mnemonic = "st " ;
  for (i=0;i<nfsaves;i++){
    fsaves[i].narg = 3;
...
/*Wasting one Iop here - could set condition code with addcc*/
  queue_barrier();
