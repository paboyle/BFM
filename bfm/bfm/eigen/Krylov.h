...
#include <util/lattice/bfm_qdp.h>*/

//Change bfm_qdp -> bfm_qdp
//Change bfm_qdp<S> dop to reference bfm_qdp<S> &dop
//Remove dwfa_setup
...
class Krylov : public bfm_internal<S> 
{
  public:
    ...
    Matrix<double> H;	///Hessenberg matrix
    ...
    S ch_rad; ///Spectral radius
    S ch_off; ///Spectral offset (ie. find eigenvalues of magnitude less than this)
    S ch_shift; ///Shift the peak

    multi1d<double> shift_extra;
  ...
  protected:
    ...
    void Check();
    void Reset();
    void ImplicitRestart(	int TM, vector<double> &evals,  vector<vector<double> > &evecs);
    void TestConv(int SS, vector<double> &tevals, vector<vector<double> > &tevecs);
    void True_Residual( void );
    int Lanczos_Factor(int start, int end);
    void G5R(bfm_fermion result, bfm_fermion input);
    void Abort(int ff, vector<double> &evals,  vector<vector<double> > &evecs);
    void Shift(Matrix<double> &Q, vector<double> shifts);
    ...
    void times_real(multi1d<bfm_fermion> &q, Matrix<double> &Q, int N);
    void times(multi1d<bfm_fermion> &q, Matrix<double> &Q, int N);

    void axpy(bfm_fermion r, S a, bfm_fermion x, bfm_fermion y);
    S axpy_norm(bfm_fermion r, S a, bfm_fermion x, bfm_fermion y);
    void axpby(bfm_fermion r, S a, bfm_fermion x, S b, bfm_fermion y);
    S axpby_norm(bfm_fermion r, S a, bfm_fermion x, S b, bfm_fermion y);
    ...
    void scale(bfm_fermion x, complex<S> s); 
    S norm(bfm_fermion x);
    S norm2(bfm_fermion x);
    void gamma5(bfm_fermion x); 
    S innerProduct_real(bfm_fermion x, bfm_fermion y);
    complex<S> innerProduct(bfm_fermion x, bfm_fermion y);
    double Chebyshev(double x);
    void Chebyshev(bfm_fermion input, bfm_fermion &v1);
    double qpoly(double alpha, double beta, double x);
    void qpoly(S beta, S alpha, bfm_fermion input, bfm_fermion &result);
...
/*
template <class S> 
Krylov<S>::Krylov(bfm_qdp<S> &dwf): dop(dwf)
{
  con = 0;
...
void Krylov<S>::Run()
{
  ...
  for(int it = 0; it < this->maxits && (this->con < this->get); ++it)
  {
    ...
    std::vector<double> tevals(this->M - lock_num );
    std::vector<std::vector<double> > tevecs(this->M - lock_num);
  ...
  if(this->kr == lan){
    SymmEigensystem<double>(this->H, this->evals, this->evecs, this->convQR);}
QDPIO::cout << "herdsvgsdfsdge" << endl;
  Check();
...
template <class S> void Krylov<S>::Shift(Matrix<double> &Q, vector<double> shifts){
  ...
  for(int i=0;i<P;i++){
    ...
    Householder_vector<double>(ck, 0, 2, v, beta);
    Householder_mult<double>(H,v,beta,0,lock_num+0,lock_num+2,0);
    Householder_mult<double>(H,v,beta,0,lock_num+0,lock_num+2,1);
    ///Accumulate eigenvector
    Householder_mult<double>(Q,v,beta,0,lock_num+0,lock_num+2,1);
    int sw = 0;
    ...
    for(int k=lock_num+0;k<M-2;k++){
      ...
      Householder_vector<double>(ck, 0, 2-sw, v, beta);
      Householder_mult<double>(H,v, beta,0,k+1,k+3-sw,0);
      Householder_mult<double>(H,v, beta,0,k+1,k+3-sw,1);
      ///Accumulate eigenvector
      Householder_mult<double>(Q,v, beta,0,k+1,k+3-sw,1);
    }
  }
}

///Converge on invariant subspace and quit "elegantly"
template <class S> void Krylov<S>::Abort(int ff, vector<double> &evals,  vector<vector<double> > &evecs)
{
  QDPIO::cout << "Check for converged eigenvectors before aborting" << endl;
  QDPIO::cout << "Subspace dimension " << ff << " < " << this->M << endl; 

	int lock_num = this->lock ? this->con : 0;
  std::vector<double> tevals(ff-lock_num);
  std::vector<std::vector<double> > tevecs(ff-lock_num);
...
template <class S> int Krylov<S>::Lanczos_Factor(int start, int end){
  ...
  if(start == 0){
    ...
    this->H( (double)real(alpha),0,0);							//bq[0] . bf =  bq[0]^dag A bq[0] - alpha |bq[0]|^2 = 0
  }
  int re = 0;
  start--;

  beta = this->axpy_norm(this->bf[0],0.0,this->bf[0],this->bf[0]);		//|bf|^2
  double sqbt = sqrt(beta);
  ...
  for(int j=start+1;j<end;j++){
    ...
    double fnorm = this->axpy_norm(this->bf[0] , -1.0*real(alpha), this->bq[j], this->bf[0]);		//bf = A bq[j] - beta bq[j-1] - alpha bq[j]

    re = 0;
    double bck = sqrt( real( conj(alpha)*alpha ) + beta );
    beta = fnorm;
    sqbt = sqrt(beta);
    ...
    while( re == this->ref || (sqbt < this->rho * bck && re < 5) ){
      ...
      for(int l=0;l<j+1;l++){
        double nrm = norm2(this->bq[l]);
        this->axpy(this->tmp1,0.0,this->bq[l],this->bq[l]); this->scale(this->tmp1,(complex<S>)bex[l]); 	//tmp1 = V[j] bex[j]
        this->axpy(this->tmp2,1.0,this->tmp2,this->tmp1);					//tmp2 += V[j] bex[j]
      }

      //bf = bf - V V^dag bf.   Subtracting off any component in span { V[j] } 
      double btc = this->axpy_norm(this->bf[0],-1.0,this->tmp2,this->bf[0]);
      alpha = alpha + bex[j];	      sqbt = sqrt(real(btc));	      
      double nmbex = 0;for(int k=0;k<j+1;k++){nmbex = nmbex + real( conj(bex[k])*bex[k]  );}
      bck = sqrt( nmbex );
      re++; 
    } 
...
template <class S> void Krylov<S>::True_Residual( void ){
  ...
  std::vector<double> mevals(this->M);
  std::vector<std::vector<double> > mevecs(this->M);
  Wilkinson<double>(this->H, mevals, mevecs, this->convQR);
...
void Krylov<S>::TestConv(int SS, vector<double> &tevals, vector<vector<double> > &tevecs)
{
  ...
  for(int i = SS - lock_num - 1; i >= SS - this->K && i >= 0; --i)
  {
    ...
    if(diff < this->conv)
    {
      Matrix<double> Q(this->M);

			if(this->lock)
			{
				bool herm = true; 
				//if(this->kr == arn){ herm = false;}  
				Deflate<double>::Lock(this->H, Q, tevals[i], this->con, this->convQR, SS, herm);
				//if(this->kr == arn){
...
template <class S> 
void Krylov<S>::ImplicitRestart(int TM, vector<double> &evals,  vector<vector<double> > &evecs)
{
  ...
  std::vector<double> shifts(this->P + this->shift_extra.size());
  ...
  double beta = this->H(ff, ff-1); 
  double sig = Q(TM - 1, ff - 1);
...
///q(Q)= 2*( Q^2/s^2 ) - ( 1+r ) I
template <class S> void Krylov<S>::qpoly(S beta, S alpha, bfm_fermion input, bfm_fermion &result){
  ...
  this->axpby(result, 2/amb, result, -apb/amb, input);
...
template <class S> 
complex<S> Krylov<S>::innerProduct(bfm_fermion x, bfm_fermion y)
{
  complex<S> dot;
#pragma omp parallel 
  {
...
template <class S> 
S Krylov<S>::innerProduct_real(bfm_fermion x, bfm_fermion y)
{
  S nrm;
#pragma omp parallel 
  {
...
template <class S> 
S Krylov<S>::norm2(bfm_fermion x)
{
  S nrm;
#pragma omp parallel 
  {
...
template <class S> 
S Krylov<S>::norm(bfm_fermion x)
{
  return sqrt(norm2(x));
}

template <class S> 
void Krylov<S>::scale(bfm_fermion x, complex<S> s)
{ 
#pragma omp parallel 
  {
#pragma omp for 
...
template <class S> 
S Krylov<S>::axpby_norm(bfm_fermion r, S a, bfm_fermion x, S b, bfm_fermion y)
{
  S sum = 0;
#pragma omp parallel 
  {
...
template <class S> 
void Krylov<S>::axpby(bfm_fermion r, S a, bfm_fermion x, S b, bfm_fermion y)
{
#pragma omp parallel 
  {
#pragma omp for 
...
template <class S> 
S Krylov<S>::axpy_norm(bfm_fermion r, S a, bfm_fermion x, bfm_fermion y)
{
  S sum;
#pragma omp parallel 
  {
...
template <class S> 
void Krylov<S>::axpy(bfm_fermion r, S a, bfm_fermion x, bfm_fermion y)
{
#pragma omp parallel 
  {
#pragma omp for 
...
/// q -> q Q
template <class S> void Krylov<S>::times(multi1d<bfm_fermion> &q, Matrix<double> &Q, int N){
...
void Krylov<S>::ObliqueProj(bfm_fermion x, bfm_fermion b)
{
  ...
  std::vector<complex<double > > Wb(this->K);
  ...
  Matrix<complex<double > > WAW(this->K);
  ...
  std::vector<complex<double > > WAWWb(this->K);
  print(Wb);
...
/// q -> q Q
template <class Float> void Krylov<Float>::times_real(multi1d<bfm_fermion> &q, Matrix<double> &Q, int N)
{
	Float **V;
	V = new Float* [N];
...
template <class S> void Krylov<S>::Check(){
	...
	vector<double> goodval(this->get);
