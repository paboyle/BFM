...
struct processor *create_alpha264s (void)
{
  ...
  proc->CacheLine = 64; /*32 byte cache line*/
  proc->npipes=4;
  ...
  pipes[2].issuegroups = IALUMASK | BRCHMASK | LOADMASK  | FSTRMASK
  ...
  create_alpha264s_instructions(proc);
  return(proc);
}
...
void create_alpha264s_instructions( struct processor *proc)
{
  ...
  cache[WRITE_HINT].mnemonic = "wh64 "; /*Write hint - destructive*/
...
void check_iterations_alpha264s(int cntreg,int retno)
{
