...
{
  public:
    Krylov_5d(bfm_qdp<S> &dwf) : Krylov<S>(dwf){}
    //Krylov_5d(bfm_qdp<S> &dwf, cps::LancArg &lanc_arg) : Krylov<S>(dwf, lanc_arg) {}
    virtual ~Krylov_5d() {}
...
void Krylov_5d<S>::init()
{
  ...
  QDPIO::cout << "Krylov_5d: initializing" << endl;
...
#pragma omp parallel 
  {
#pragma omp for 
    for(int i = 0; i < this->dop.nthread; ++i) 
		{
...
void Krylov_5d<S>::eigs_to_qdp(multi1d<multi1d<LatticeFermion> > &con_vecs, multi1d<Complex> &con_vals)
{
	...
	con_vecs.resize(Nev); 
	for(int i = 0; i<Nev; i++){
	  con_vecs[i].resize(this->dop.Ls);
	  for(int s=0;s<this->dop.Ls;s++){
	    con_vecs[i][s]=zero;
	  }
	}
	con_vals.resize(Nev);
...
{
  public:
    Lanczos_5d(bfm_qdp<T> &dwf) : Krylov_5d<T>(dwf){this->kr = lan;}
    //Lanczos_5d(bfm_qdp<T> &dwf, cps::LancArg &lanc_arg) : Krylov_5d<T>(dwf, lanc_arg) {this->kr = lan;} //Jasper
    virtual ~Lanczos_5d(){}
    //Jasper
