...
struct processor *create_alpha064 (void)
{
  ...
  create_alpha064_instructions(proc);
  return(proc);
}

void create_alpha064_instructions( struct processor *proc)
{
  int i,j;
...
int start_loop_alpha064(int counter)
{
...
void stop_loop_alpha064(int branchno,int counter)
{
  queue_load_addr(counter,Hdecrement,counter);
  make_inst(BRCHPIPE,BRANCH_GT,counter,branchno);
}
void check_iterations_alpha064(int cntreg,int retno)
{
  make_inst(BRCHPIPE,BRANCH_LE,cntreg,retno);      /*Branch if cntreg <= 0*/
}
/* ...} */

void directive_print_alpha064(struct instruction *i)
{
    switch ( i->code ) {
    case Target:
      printf("%s:\n",make_label(i->args[0]));
...
void queue_cmovge_alpha064(int cond,int src,int dst)
{
  make_inst(IALUPIPE,CMOVGE,dst,cond,src);
}

void cache_print_alpha064(struct instruction *i)
{
